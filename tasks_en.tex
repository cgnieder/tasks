...
\needpackage{xparse} and \pkg{xtemplate}.%  and \needpackage{l3keys2e} which are
...
\begin{options}
  ...
  \keyval{counter-format}{counter specs}\Default{\cs*{alph}\code{*)}}
...
\begin{example}
  \begin{tasks}[counter-format=(\roman*),label-width=4ex](2)
    \task \Sample
    \task \sample\ \sample
    \task \sample
    \task \Sample
  ...
  \end{tasks}
\end{example}
% \begin{tasks}[counter-format=(\roman*),label-width=4ex](3)
%  \task \Sample
%  \task \sample\ \sample
%  \task \sample
%  \task \Sample
...
\begin{example}
  ...
  \settasks{
    counter-format = \theexercise.\arabic* ,
    item-indent    = 2em ,
    label-width    = 2em ,
    label-offset   = 0pt
  }
...
\begin{example}
  % preamble:
  % \usepackage{dingbat}
  \NewTasks[label=\footnotesize\leftthumbsup,label-width=15pt]{done}[*]
  \begin{done}
    * First task
    * Second task
  \end{done}
